...
func startClaimParametersGenerator(ctx context.Context, config *configType) error {
	...
	if _, err := gpuClaimParametersInformer.AddEventHandler(newGpuClaimParametersHandler(ctx, config.clientsets.core, dynamicClient)); err != nil {
...
func createOrUpdateResourceClaimParameters(clientset kubernetes.Interface, gpuClaimParameters *intelcrd.GpuClaimParameters) error {
	namespace := gpuClaimParameters.Namespace
